...
import { Calendar, ClipboardList, FileText, ExternalLink } from "lucide-react";
...
const GradeCard: React.FC<GradeCardProps> = ({ grade, accent }) => {
  const cardContent = (
    <Card
      className={`border-l-4 transition-all duration-200 ${grade.link ? "cursor-pointer hover:shadow-lg hover:scale-[1.02] hover:bg-gray-50/50" : ""} ${grade.status === "late"
        ? "border-l-red-500"
        : accent === "blue"
          ? "border-l-blue-500"
          : "border-l-green-500"
  ...
  if (grade.link) {
    return (
      <Link href={grade.link} className="block">
        {cardContent}
      </Link>
    );
  }
